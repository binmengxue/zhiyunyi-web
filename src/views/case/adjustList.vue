<template>
  <div class="app-container">
    <Search3 @seachEvent="seach"  @restEvent="rest"   :queryParams='this.queryParams'></Search3>
    ...
    <el-table v-loading="loading" :data="userList"  :row-class-name="tableRowClassName"  stripe  element-loading-text="拼命加载中" @selection-change="handleSelectionChange" @sort-change="sortChange"  :default-sort = "{prop: 'distributionStartTime', order: 'descending'}">
      ...
      <el-table-column align="center" label="操作" width="100" style="position:relative;" fixed="left">
        <template slot-scope="scope">
          <router-link v-show="scope.row.caseStatus!=3 && scope.row.caseStatus!=4" :to="'/casedetail/' + scope.row.caseId + scope.row.customerName" class="link-type">
      ...
      <el-table-column prop="currentCustomerState" align="center" label="债务人跟进状态" width="150">
        <template slot-scope="scope">
          {{getFileItem(scope.row.currentCustomerState,currentCustomerStateList)}}
        </template>
      </el-table-column>
    ...
    <el-dialog title="留案申请" :visible.sync="leaveCaseOpen" width="600px" append-to-body>
      <el-form ref="leaveFormDialog"   :model="leaveForm" :rules="leaveRules" label-width="100px">
        <div class="el-formDiv">
            ...
            <el-form-item label="案件到期日">
                <div>{{leaveForm.distributionEndTime}}</div>
            </el-form-item>
            <el-form-item label="申请到期日" prop="stayEndTime">
                 <el-date-picker type="datetime" :picker-options="pickerOptions" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" v-model="leaveForm.stayEndTime" style="width:250px"></el-date-picker>
            </el-form-item>
...
<script>
import {treeselect,caseManagerList,uploadAttachmentFile,caseInfoCaseDistribution,caseEmployeeList ,updateCaseStatus,addCaseLeave,frozenOrUnfrozenCase,caseManagerExport,batchCaseDistribution} from "@/api/case/index";
import { getToken } from "@/utils/auth";
import  Search3  from "@/views/case/search3.vue";
import {getcasestatus,getconfig,getcaseSource,getadjustResult,getadjustType,getCurrentCustomerState} from '@/utils/statusInfo'
...
export default {
  ...
  methods: {
    ...
    handleLeave(){
      ...
      this.leaveForm.distributionEndTime = this.multipleTable[0].distributionEndTime?
        this.multipleTable[0].distributionEndTime : "";
